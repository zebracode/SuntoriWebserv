...
exports.renderIndex = function (req, res, next) {
	if (typeof req.body.amount !== 'undefined' && req.body.payment_status === '000') {
		...
		Balance.findOne({ userId: req.user._id }, function (err, balance) {
			if (err) {
				...
				if (balance === null) {
					...
					bl.save(function (err, balance) {
						if (err) {
							return next(err);
						} else {
							// Save statement
							saveStatement(amount, amount);
					...
					Balance.findOneAndUpdate({ userId: req.user._id }, { balanceAmt: newBalance },
						function (err, balance) {
							if (err) {
								...
								// Save statement
								saveStatement(amount, amount + Number(balance.balanceAmt));
		...
		// Save statement function
		function saveStatement(amount, balance) {
			var statement = new Statement({
				user: req.user,
				name: "เติมงานจาก 2P2C",
				amountIn: amount,
